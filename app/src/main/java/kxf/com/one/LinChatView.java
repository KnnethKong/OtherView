...
public class LinChatView extends View {
    ...
    public LinChatView(Context context, AttributeSet attrs) {
        super(context, attrs, 0);
    }

    public LinChatView(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.myLinChatView, defStyleAttr, 0);
        int count = typedArray.getIndexCount();
        for (int i = 0; i < count; i++) {
            int attr = typedArray.getIndex(i);
            switch (attr) {
                case R.styleable.myLinChatView_bgcolor:
                    bgcolor = typedArray.getColor(attr, bgcolor);
                    break;
                case R.styleable.myLinChatView_interval:
                    interval = (int) typedArray.getDimension(attr, TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_PX, interval, getResources().getDisplayMetrics()));
                    break;
                case R.styleable.myLinChatView_isScroll:
                    isScroll = typedArray.getBoolean(attr, isScroll);
                    break;
                case R.styleable.myLinChatView_linecolor:
                    linecolor = typedArray.getColor(attr, linecolor);
                    break;
                case R.styleable.myLinChatView_xylinewidth:
                    xylinewidth = (int) typedArray.getDimension(attr, TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_PX, xylinewidth, getResources().getDisplayMetrics()));
                    break;
                case R.styleable.myLinChatView_xytextcolor:
                    xytextcolor = typedArray.getColor(attr, xytextcolor);
                    break;
                case R.styleable.myLinChatView_xytextsize:
                    xytextsize = (int) typedArray.getDimension(attr, TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_PX, xytextsize, getResources().getDisplayMetrics()));
                    break;
            }
        }
        typedArray.recycle();
        ...
        for (int i = 0; i < xValue.size(); i++) {
            ...
            if (x >= xOri) {//只绘制从原点开始的区域
                ...
                if (i == selectIndex - 1) {
                    ...
                    float f1=  x-xValueRect.width()/2-dpToPx(3);
                    float f2=   yOri + xylinewidth + dpToPx(1);
                    float f3= x + xValueRect.width() / 2 + dpToPx(3);
                    float f4=yOri + xylinewidth + dpToPx(2) + xValueRect.height() + dpToPx(2);
                    RectF rectF = new RectF(f1,f2,f3,f4);
                    canvas.drawRoundRect(rectF , dpToPx(2), dpToPx(2), xyTextPaint);
                } else {
                    canvas.drawText(text, 0, text.length(), x - rect.width() / 2, yOri + xylinewidth + dpToPx(2) + rect.height(), xyTextPaint);
                }
            }
